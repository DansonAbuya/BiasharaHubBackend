-- BiasharaHub Services: service_categories table and seed (provider picks category then online/physical)
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format('CREATE TABLE IF NOT EXISTS %I.service_categories (
            category_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            name VARCHAR(100) NOT NULL UNIQUE,
            display_order INTEGER NOT NULL DEFAULT 0
        )', r.schema_name);
        EXECUTE format('CREATE INDEX IF NOT EXISTS idx_service_categories_display_order ON %I.service_categories(display_order)', r.schema_name);
        -- Seed all possible service categories (idempotent)
