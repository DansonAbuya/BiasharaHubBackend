-- New tenants get service_categories, service_offerings.service_category_id, and service_appointments
...
BEGIN
    ...
    EXECUTE format('CREATE TABLE IF NOT EXISTS %I.shipments (
        ...
        assigned_courier_id UUID REFERENCES %I.users(user_id),
        delivery_mode VARCHAR(50) NOT NULL DEFAULT ''SELLER_SELF'' CHECK (delivery_mode IN (''SELLER_SELF'', ''COURIER'', ''RIDER_MARKETPLACE'', ''CUSTOMER_PICKUP'')),
    ...
    EXECUTE format('CREATE TABLE IF NOT EXISTS %I.service_offerings (
        ...
        service_category_id UUID REFERENCES %I.service_categories(category_id) ON DELETE SET NULL,
